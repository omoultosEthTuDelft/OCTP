                        ...
                        double *Rzz[], double *Fxx[], double *Fyy[], double *Fzz[]) 
{
     ...
     * @return 0 if the timestep was parsed successfully, 1 if end of file was reached
     */
    char buffer[MAXLINE];
                ...
                const double Fxx[], const double Fyy[], const double Fzz[]) 
{
    ...
    // start an openmp parallel region and give each thread its own copy of the
    // g[] array of length BinNum
    // for (i = 0; i < BinNum; i++) {
    //     g[i] = 0.0;
    // }
    memset(g, 0, BinNum * sizeof(double));

    #pragma omp parallel for private(i, j, k, Dx, Dy, Dz, Rij, Ff) reduction(+:g[:BinNum]) 
    for(i=0;i<NumberOfParticles;i++) {
...
void average_rdf(double gavg[], const long nTimesteps)
{
    /**
     * @brief Average the rdf over all timesteps
     * @param gavg Array to store the averaged rdf
...
void write_rdf(char* base_name, double g[], const long Timestep, const long nTimesteps, const double halfbox)
{
    /**
     * @brief Write the rdf to a file
     * @param fp File pointer to the file to write to
     * @param g Array of rdf values
     */
    long i;
    double R;
    FILE *fp;
    ...
    average_rdf(g, nTimesteps);

    sprintf(fname, "%s_%ld.dat", base_name, Timestep);
    fp = fopen(fname, "w");
    if (fp == NULL) {
    ...
    fprintf(fp, "# Number of timesteps used: %ld\n", nTimesteps);
    fprintf(fp, "# R, g(R)\n");
    ...
    printf("Writing rdf for timestep %ld after %ld samples\n", Timestep, nTimesteps);
    fclose(fp);
...
int main(int argc, char *argv[])
{
    FILE *fp;
    int eof;
    long NumberOfParticles, k, timestep, nTimesteps, nWrite=0, minTimestep=0;
    double Temp, c, Box, *Rxx, *Ryy, *Rzz, *Fxx, *Fyy, *Fzz, *g, *gavg;
    ...
    while (feof(fp) == 0) { 
        ...
        if (nWrite != 0 && nTimesteps == nWrite) {
            write_rdf(argv[3], gavg, timestep, nTimesteps, Box/2.0);
            nTimesteps = 0;
            memset(gavg, 0, BinNum * sizeof(double));
        }
    }
